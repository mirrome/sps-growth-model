...
import { simulate, buildEqualPolicy, buildGrowthBaselinePolicy } from './simulate'
...
function singleLineScenario(overrides: LineOverrides = {}): Scenario {
  ...
  return {
    schemaVersion: 1,
    meta: { name: 'Test', horizonYears: horizon, isIllustrative: true },
    corporate: {
      taxRate: 0.3,
      rD: 0.06,
      rE: 0.12,
...
// ---------------------------------------------------------------------------
// Integration tests for buildGrowthBaselinePolicy
//
// These three assertions guard the PM-supplied growth arrays against three
// common failure modes:
//   1. Debt-gate breach in year 0 (capex > pre-capex OCF when no debt available)
//   2. Rock supply constraint violation in any year
//   3. Policy not actually producing a growth story (revenue year-10 < 2× year-0)
//
// If any test fails, flag the failure mode to the PM with the exact numbers
// so the arrays can be retuned. Do not weaken the assertion or commit with
// a failing test.
// ---------------------------------------------------------------------------
describe('buildGrowthBaselinePolicy — integration guards', () => {
  ...
  it('year-0 total capex does not exceed pre-capex OCF (debt-gate gate closed in 2026)', () => {
    ...
    // Pre-capex OCF = FCF[0] + capex[0]  (FCF already has capex deducted)
