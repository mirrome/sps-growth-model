...
export const MATH_SECTIONS: MathSection[] = [
  ...
  },
  {
    ...
    prose:
      "Each dollar spent on research and development builds up a pipeline of products in development, measured in units of the annual revenue those products will generate once launched and fully ramped. The productivity parameter translates research dollars into latent revenue, with typical specialty chemicals values in the range of two to four dollars of eventual annual revenue per dollar of research spending.\n\nThe pipeline does not convert instantly. Products require a maturation period before they can be commercialized, typically two years for line extensions in established categories and up to four years for genuinely new chemistry. Once mature, the pipeline converts to launched revenue at a steady annual rate, reflecting the typical pace at which a product rolls out across customers and geographies. The conversion adds to the line's revenue without inflating physical volume, because new products expand the value captured per kiloton rather than the kilotons produced.\n\nImportantly, products committed to development in earlier years continue to launch on their original schedule even if the pipeline is no longer being replenished, because the development work has already been done. This means that a line that stops investing in research and development can still see launched revenue for several years from prior commitments, after which the contribution from new products fades to zero.\n\nThe timing convention the model uses is that research spending in year t first contributes to the pipeline at the end of year t, and first produces launched revenue in year t plus lambda plus one. For a line with a two-year maturation lag, a dollar of research spent in 2026 first shows up as launched revenue in 2029. This one-year offset reflects the practical reality that a dollar of research committed in January of a given year is still being turned into a product-development plan during that year; the pipeline effectively gains the committed value at year end. The starting pipeline at the beginning of the plan is separately modeled and begins launching earlier, based on research already committed before the horizon.",
  },
  {
    id: 'price',
